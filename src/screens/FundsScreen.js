...
export default function FundsScreen() {
  ...
  const close = () => { setOpen(false); setEditing(null); setName(''); setType('physical'); setBalance(''); };
  ...
  return <View style={styles.container}>
    ...
    <FlatList data={accounts} keyExtractor={(item) => item.id} contentContainerStyle={[styles.list, { paddingBottom: insets.bottom + 72 }]} renderItem={({ item }) => (
      <Pressable style={styles.row} onPress={() => startEdit(item)}>
        ...
        <Text style={styles.amount}>{fmtCurrency(item.balance, settings.currency)}</Text>
      </Pressable>
    )} />
    ...
    <Sheet visible={open} onClose={close}>
      ...
      <Text style={styles.label}>Type</Text><View style={styles.types}>{TYPES.map((item) => <Pressable key={item.key} style={[styles.typeButton, type === item.key && styles.typeActive]} onPress={() => setType(item.key)}><Text style={[styles.typeButtonText, type === item.key && styles.typeActiveText]}>{item.label}</Text></Pressable>)}</View>
...
const styles = StyleSheet.create({
  container:{flex:1,backgroundColor:theme.colors.bg,padding:theme.spacing(4)},list:{gap:theme.spacing(2)},row:{flexDirection:'row',alignItems:'center',gap:theme.spacing(3),backgroundColor:theme.colors.surface,borderWidth:1,borderColor:theme.colors.line,borderRadius:theme.radius.md,padding:theme.spacing(3)},icon:{width:40,height:40,borderRadius:20,backgroundColor:theme.colors.accentSoft,alignItems:'center',justifyContent:'center'},info:{flex:1},name:{fontWeight:'600',color:theme.colors.ink},type:{fontSize:12,color:theme.colors.inkSoft,marginTop:2},amount:{fontFamily:theme.fonts.mono,fontWeight:'700',color:theme.colors.ink},add:{position:'absolute',right:20,flexDirection:'row',gap:6,alignItems:'center',backgroundColor:theme.colors.accent,paddingHorizontal:16,height:48,borderRadius:24},addText:{color:theme.colors.surface,fontWeight:'600'},title:{fontSize:18,fontWeight:'600',marginBottom:16,color:theme.colors.ink},label:{fontSize:12,color:theme.colors.inkSoft,marginBottom:4},types:{flexDirection:'row',gap:6,marginBottom:12},typeButton:{flex:1,paddingVertical:9,alignItems:'center',borderWidth:1,borderColor:theme.colors.line,borderRadius:theme.radius.sm},typeActive:{backgroundColor:theme.colors.accent,borderColor:theme.colors.accent},typeButtonText:{fontSize:11,color:theme.colors.inkSoft},typeActiveText:{color:theme.colors.surface},input:{borderWidth:1,borderColor:theme.colors.line,backgroundColor:theme.colors.surface,borderRadius:theme.radius.md,padding:12,fontSize:15,marginBottom:12},actions:{flexDirection:'row',gap:12},button:{flex:1,alignItems:'center',paddingVertical:12,borderRadius:theme.radius.md},ghost:{borderWidth:1,borderColor:theme.colors.line},primary:{backgroundColor:theme.colors.accent},primaryText:{color:theme.colors.surface,fontWeight:'600'}
});
